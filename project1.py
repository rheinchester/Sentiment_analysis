...
def perceptron_single_step_update(
    ...
    dot_product =np.dot(current_theta, feature_vector)
    if (dot_product+current_theta_0)*label <= epsilon:
...
def perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    theta, theta_0 = np.zeros(np.shape(feature_matrix)[1]), 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    # raise NotImplementedError
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):#MIT implemented
        # for i in range(feature_matrix.shape[0]):#MIT implemented
            theta, theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], theta, theta_0)
            sum_theta += theta
            sum_theta_0 += theta_0
            counter+=1
    # Your code here
    return (sum_theta/(len(labels)*T), sum_theta_0/(len(labels)*T))
...
def pegasos_single_step_update(
    ...
    """
    # Your code here
    dot_product =np.dot(theta, feature_vector) + theta_0
    term1 = (1 - (eta*L)) * theta
    term2 = eta * label * feature_vector
    if (dot_product*label) <= 1:
        theta =  (term1 + term2)
...
def bag_of_words(texts, remove_stopword=False):
    ...
    Returns:
        ...
        integer `index`.
    """
    # Righteous looking code lol.
    if remove_stopword:
        stopword = load_text_file('stopwords.txt')
...
def extract_bow_feature_vectors(reviews, indices_by_word, binarize=True):
    ...
    if binarize:
        binarized_feature_matrix = feature_matrix.copy()
        binarized_feature_matrix = np.where(binarized_feature_matrix >= 1, 1, 0)
        return binarized_feature_matrix
...
def load_text_file(file_path):
    """
    Open a text file and split its contents into lines.
